...
import { Shield, Award, Lock, Mountain, Heart, Palette, Search, UtensilsCrossed, Building2, Gem, ChevronDown } from 'lucide-react';
...
export function Home() {
  ...
  const handleCategoryClick = (category: string) => {
    navigate(`/experiences?category=${category}`);
  };
    ...
    <div className="min-h-screen">
      <section className="relative h-screen flex items-center justify-center text-white">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-red-600 rounded-lg mb-8 relative">
          ...
          <p className="text-xl md:text-2xl font-light mb-12 text-gray-200 tracking-wide">
            From alpine peaks to artisan workshops — invite-only hosts you won't find elsewhere
          </p>

          <div className="max-w-2xl mx-auto mb-8">
            <form onSubmit={handleSearch} className="flex items-center bg-white rounded-xl shadow-lg hover:shadow-red-500/20 transition-shadow duration-300">
              ...
              <input
                ...
                className="flex-1 py-4 px-2 text-slate-800 placeholder-slate-400 outline-none bg-transparent font-light"
              />
              <button type="submit" className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-r-xl font-light transition-colors">
                Search
              </button>
            </form>
          </div>
          ...
          <div className="flex flex-wrap gap-3 justify-center mb-12">
            <button onClick={() => handleCategoryClick('nature')} className="inline-flex items-center gap-2 px-5 py-2 bg-white/10 backdrop-blur-sm rounded-full text-sm font-light hover:bg-white/20 transition-all">
              <Mountain size={16} />
              Alpine
            </button>
            <button onClick={() => handleCategoryClick('culture')} className="inline-flex items-center gap-2 px-5 py-2 bg-white/10 backdrop-blur-sm rounded-full text-sm font-light hover:bg-white/20 transition-all">
            ...
            <button onClick={() => handleCategoryClick('wellness')} className="inline-flex items-center gap-2 px-5 py-2 bg-white/10 backdrop-blur-sm rounded-full text-sm font-light hover:bg-white/20 transition-all">
              <Gem size={16} />
              Luxury
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link
              to="/experiences"
              className="inline-block px-12 py-4 bg-white text-slate-800 rounded-lg hover:bg-opacity-90 transition-all text-lg font-light"
            >
              Explore Experiences
            </Link>
            <Link
              to="/corporate"
              className="inline-block px-8 py-4 border border-white border-opacity-40 text-white rounded-lg hover:bg-white hover:bg-opacity-10 transition-all text-base font-light"
            >
              Corporate Retreats
            </Link>
          </div>
